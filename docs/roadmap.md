...
- **[U] Outstanding:** open it in your Edge (`npm run dev` in `apps/shell`, then http://127.0.0.1:4173, and `?view=2d` for the fallback). Judge look and frame rate, decide which effects stay (bloom, sensor looks, the scope mask) and check reduced motion.
