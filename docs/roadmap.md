...
**Status, 2026-09-25: built on branch `m5-shell`; [T] and [A] done, [U] outstanding.** `apps/shell` is a stripped fork of God's Eye View (`f01b6a5`), recorded step by step in `apps/shell/UPSTREAM.md`. It keeps the look (globe with scope mask, HUD, command dock, celestial ring, split-flap readouts, Scenes, Data Layers and Display panels) and contains only Nexus data: the six oil anchors and two corridors, one guided tour, and the pack readouts. Added: the action layer with global-context mode, share links with hostile-hash tests, the 2D Natural Earth fallback, and `scripts/check_shell.py`.
- **[T]** 816 unit tests pass (3 skipped), `tsc` clean, eight Playwright smoke tests pass (renders with no console errors; oil layers add and remove; readouts; action layer, tour and global context; hostile and valid share hashes; the fallback by request and by missing WebGL). One `@cesium/engine`. Release-mode licence gate: 0 errors over the shell build. The build is 405 files, 17 MB, against 423 files and 31.2 MB for the untouched reference; it contacts only localhost, Esri imagery and the Re:Earth terrain service.
- **[A]** Screenshots of the opening view and the fallback were taken in my sandboxed Edge (not your GPU) and match the look of the reference minus the removed panels.
- **[U] Done, 2026-09-25:** the user ran it in their Edge. Look "clean, sharp and very smooth", performance smooth, the visual effects and the scope mask kept (the scope mask "keeps things in focus"). Reduced motion showed no visible difference: the code honours it only in the logo, the split-flap readouts and one status animation, so the opening flight, tour flights and the loading pulse still move. Selecting an anchor shows its name and "representative label point"; the Location tray flies cleanly but is sparse.
- **Not done:** nothing calls the action layer except the tests (the panels still call their own functions); no Nexus entity search; the selection card is Cesium's stock info box; Esri and OSM imagery are not cleared for a public build.

**User feedback from the M5 check, to schedule where it fits (not for now):**
- **Settings menu (M6).** Move the visual presets (effects, sensor looks) out of the dock into a separate settings screen reached from a menu, since more information will share that space; decide what else belongs there. The effects and the scope mask stay.
- **Data Layers polish (M6).** Map labels overlap when anchors are close (Hormuz and Yanbu; Malacca, Lombok and Sunda); the layer rows can be improved. Also, on windows shorter than about 700 CSS pixels (a 125%-scaled display), opening Data Layers hides the Scenes panel entirely (upstream's panel-stack rule, tuned for long panels); closing it brings Scenes back. With two short rows it need not hide.
- **Corridor clarity (M7).** The two dashed lines are the Suez Canal / SUMED corridor and the Myanmar to China crude pipeline (Maday Island near Kyaukphyu to Kunming). Both are straight illustrative segments between sourced endpoints, and the caveat on each says so; the tour and a legend should say "modelled, not observed" on the map itself.
- **Richer Location tray (M6).** It lists only the six oil stops in two groups; it grows with the entities and chapters.
- **Reduced motion (M6).** Honour it for the opening flight, tour flights and the loading pulse (jump rather than fly).
- **Planes and boats (needs a decision, after M8).** The user finds live aircraft and vessels add a lot. This reverses the earlier decision to strip live tracking (`docs/decisions.md`) and conflicts with the static public release (M9), which cannot show live data. Before scheduling: which feeds (AIS for vessels is the relevant one for oil), their terms, keys and cost, and whether it is a local-only feature. Vessel positions are observations, so they would be a separate labelled layer, never mixed into sourced anchors. Not authorised: monitoring or scheduled fetching.
