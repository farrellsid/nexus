...
- Guided oil geography with four sourced coordinate anchors, explicit point precision and direct transitions back into the evidence graph. A narrow Cesium viewer/source path adapted from God’s Eye View renders a keyless Esri globe with OpenStreetMap fallback; browsers without usable WebGL automatically receive a local Natural Earth world map. The view does not invent route geometry or map distributed/country entities to arbitrary centroids.
- Python/FastAPI API with validated fixture import and explicit domain ownership.
- Oil brief for 2025-Q3 through 2026-Q2, with selected structural baselines and developments through 2026-09-10: 13 public source records, 21 entities, 22 source-checked candidate assertions, 6 developments, 6 metric groups and 11 authored questions. Estimates, reported values, forecasts and unknowns remain distinct.
...
- An additive migration generator compares a proposed fixture with the retained database seed, refuses edits or removals, and emits a reviewable SQL migration for additions. Migration `004_expand_oil_routes.sql` carries this expansion without replacing review history.
...
1. Add sourced area/route geometry. Route geometry must remain distinct from relationship edges.
2. Deepen the oil baseline with stocks, refining capacity and additional producer/consumer series, without mixing definitions or release cadences.
3. Add a geographic review model before allowing location corrections in the UI.
4. Add evidence-bounded generated answers with citation/unknown/conflict evaluation and a separately chosen model budget.
