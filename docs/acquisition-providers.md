...
1. **Bind measurements to the new sources** (O-M07 to O-S28, O-M09 to O-S29, O-M08 to O-S30) in a normalisation release, so agreement with EIA is part of each measurement's evidence? *The user's answer, 2026-09-24: first "to be taken into consideration", then "I guess we can do the EIA binding?". Done the same day: release `nx-norm-2026-09-24-r3` binds the seven EIA-verified points to O-S28 to O-S30 (migration `012_measurement_sources.sql`, append-only `measurement_source`). Each binding carries its basis: the cross-check result and the note that it is the same publisher and survey, not an independent measurement. The bindings add to a metric's group sources, so the older prose anchors (O-S14 to O-S16) still count. Applied to the real database with a backup (`.local/backups/nexus-20260924-073804-b5330d.dump`); claims, proposals, decisions, versions and sources were unchanged.*
