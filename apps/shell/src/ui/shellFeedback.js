...
export class ShellFeedback {
  ...
  _startFeedbackTicker() {
    if (this.destroyed) return;
    if (this._feedbackTicker) return;
    this._feedbackTicker = setInterval(() => {
  ...
  destroy() {
    ...
    this.destroyed = true;
    this._stopLoadingFeedbackTicker();
    clearInterval(this._feedbackTicker);
    clearTimeout(this._toastTimer);
    this._feedbackTicker = this._toastTimer = null;
